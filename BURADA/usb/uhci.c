...
static int uhci_control_transfer(usb_device_t *dev, const usb_setup_packet_t *setup, void *data) {
    ...
    status_td->buffer = 0;
    status_td->cs     = TD_CS_ACTIVE | TD_CS_IOC | (3u << TD_CS_CERR_SHIFT) |
                         (dev->low_speed ? TD_CS_LOWSPEED : 0);
...
void uhci_poll(void) {
    ...
    uint8_t toggle_next = (uint8_t)((g_mouse_td.token >> TD_TOK_TOGGLE_SHIFT) & 1) ^ 1;
    ...
    /* NAK dahil her durumda yeniden kur (bir NAK "fare hareket etmedi"
     * anlamına gelir, hata değildir — sessizce yeniden dener). */
