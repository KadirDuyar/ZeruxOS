...
static void usb_hid_mouse_report(usb_device_t *dev, const uint8_t *data, uint32_t len) {
    ...
    /* QEMU veya gercek donanimdan ne geldigini gorelim */
    if (dx != 0 || dy != 0 || data[0] != 0) {
        serial_printf("[MOUSE] dx=%d dy=%d btn=0x%x\n", dx, dy, data[0]);
    }
